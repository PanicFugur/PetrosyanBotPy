...
class shuffle_bag:
    ...
    def __init__(self, items, ffile):
        if (isfile(join('misc', ffile))) and (os.path.getsize(join('misc', ffile)) > 0):
            with open((join('misc', ffile)),'rb') as f:
                self.items = pickle.load(f)
                print('{0}||{1} file found and loaded data from it'.format(time.asctime( time.localtime(time.time())),ffile))
        else:
            if not isfile(join('misc', ffile)):
                a = open(join('misc', ffile), 'wb')
                a.close()
    ...
    def draw(self):
        ...
        self.items.pop(-1)
        with open(join('misc', self.file), 'wb') as f:
